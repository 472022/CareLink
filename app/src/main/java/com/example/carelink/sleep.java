...
public class sleep extends AppCompatActivity {
    ...
    private void showDialog() {
        ...
        dialog.setContentView(R.layout.activity_bottom3);
        ...
        add.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                boolean result = DB.addSleep(date, ca);
                if (result) {
                    Toast.makeText(sleep.this, "Calories added successfully!", Toast.LENGTH_SHORT).show();
                    displayWaterIntake(date); // Refresh the displayed calories
                } else {
                    Toast.makeText(sleep.this, "Failed to add calories.", Toast.LENGTH_SHORT).show();
                }
    ...
    private void displayWaterIntake(String date) {
        ...
        if (cursor.moveToFirst()) {
            @SuppressLint("Range") int sleep = cursor.getInt(cursor.getColumnIndex("sleep"));
            t1.setText(sleep + "H"); // Display water intake in t1
        } else {
            t1.setText("0H"); // If no data found, show 0L
        }
